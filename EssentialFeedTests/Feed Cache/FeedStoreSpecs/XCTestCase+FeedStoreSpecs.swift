...
extension FeedStoreSpecs where Self: XCTestCase {
    ...
    func assertThatInsertDeliversNoErrorOnEmptyCache(on sut: FeedStore, file: StaticString = #filePath, line: UInt = #line) {
        ...
        let insertionError = insert((uniqueImagesFeed().local, Date()), into: sut)
    ...
    func assertThatInsertDeliversNoErrorOnNonEmptyCache(on sut: FeedStore, file: StaticString = #filePath, line: UInt = #line) {
        ...
        insert((uniqueImagesFeed().local, Date()), into: sut)
    ...
    @discardableResult
    func insert(_ cache: (feed: [LocalFeedImage], timestamp: Date), into sut: FeedStore, file: StaticString = #filePath, line: UInt = #line) -> Error? {
        ...
        sut.insert(cache.feed, timestamp: cache.timestamp) { insertionResult in
            ...
            if case let .failure(receivedInsertionError) = insertionResult {
    ...
    func deleteCache(from sut: FeedStore, file: StaticString = #filePath, line: UInt = #line) -> Error? {
        ...
        sut.deleteCachedFeed { deletionResult in
            if case let .failure(receivedDeletionError) = deletionResult {
